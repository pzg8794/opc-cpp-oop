...
void calcAvg(ifstream& inData, ofstream& outData, float& Average)
{
	...
	float  TotalGrade, TAverage=0;		
	char test, Avg, TAvg;						     //declearing grade's variables.
	float x;
	...
	while(!inData.eof())						     // Assigning a number to stop the loop.
	{
		...
		{		
		 ...
		 outData<<setw(21)<<" Exam No. "<< n <<" "<< setw(17)<< test<<'+';
		 }
		 else 
		 {
			 										       //Displaying the number of exams.
		 ...
		 if (n==1) {outData<<setw(27)<<right<<" Student No."<<setw(2)<<right<<i<<endl;}
		 else { outData<<"\n";}
		}
	  	 		
													  //Total Grade formula for five grades.
		TotalGrade= grade1+grade2+grade3+grade4+grade5; 
		Average= TotalGrade/5;							  //Average formula for four grades.
	...
	outData<<setw(70)<<right<<" __________________________________________________________";
	outData<<" \n";
	outData<<setw(26)<<" Student Name: "<< setw(45)<<" Average "<<"\n"; 
	outData<<setw(25)<< student_name <<" "<< setw(24)<<" ";
