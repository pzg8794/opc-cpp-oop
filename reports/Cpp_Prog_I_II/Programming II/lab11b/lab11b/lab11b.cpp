...
using namespace std;
  ...
  int main ()
  {
	...
	student[25].AuthorAndTitle(inData, outData);
	
	student[25].getData(inData,student,size);
	
	student[25].printOut(outData, student, size);
  }

  void  student::getData  (ifstream & inData, student  student[ ], int& size )
   {
  ...
  void  student::calcGrade (student student[ ], int i ) 
  {
  ...
  int  student::highScore (const  student  student[ ], int size ) 
   {
	   int i, highscore=0;
  ...
  void  student::printOut (ofstream & outData, const  student student[], int size )
  {
	int i=0,x;
	float TAverage=0;
  ...
  void student::AuthorAndTitle(ifstream& inData, ofstream& outData)
  {
  	outData<<"\n\n";
	outData<<"Lab Assignment # 11 " <<setw(95)<<right<< " By  Piter Garcia\n" ;
	outData<<"-------------------" <<setw(95)<<right<< "----------------" ;
	outData<<"\n\n\n";
	outData<<right<<setw(86)<< " PROGRAM ASSIGNMENT USING ARRAYS IN STRUCTURE DATA \n";
	outData<<setw(86)<<right<< " =================================================\n\n";
	outData<<"\n";
  }
   void student::ClassAverage(ofstream & outData, float TAverage )
  {
